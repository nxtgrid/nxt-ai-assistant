...
class TestGenerateSiteLayoutStep:
    ...
    async def test_deye_technology_family_defaults_to_ess_layout(self, mock_context):
        ...
        assert result.state_updates["editable_site_type"] == "ess"
        assert result.state_updates["editable_panel_config"] == "20S2P"
        mock_context.send_progress_to_user.assert_awaited()
        progress_message = mock_context.send_progress_to_user.await_args.args[0]
        assert "DEYE/ESS" in progress_message

    @pytest.mark.asyncio
    async def test_deye_packet_input_defaults_to_ess_layout(self, mock_context):
        """Deye packet input uses the ESS physical layout even below the kWp threshold."""
        mock_context.packet_inputs = {"technology_family": "deye"}
        mock_context.get_state = MagicMock(return_value=None)
        mock_context.get_parameter_value = MagicMock(
            side_effect=lambda k, d=None: mock_context.packet_inputs.get(
                k,
                {
                    "editable_total_kwp": "45.5",
                    "editable_site_type": None,
